...
TEST(stencil_omp_simd_2D5PTest, correctness) {
...
TEST(stencil_omp_simd_2D9PTest, correctness) {
  uint32_t n = 516; // nxn + 4
  auto in = generateMatrix(n);
...
TEST(stencil_omp_par_simd_2D5PTest, correctness) {
  uint32_t n = 516; // nxn + 4
  auto in = generateMatrix(n);
  ...
  stencil_2D_5P(n, in.get(), outBaseline.get());
  stencil_omp_par_simd_2D_5P(n, in.get(), outKernel.get());
...
TEST(stencil_omp_par_simd_2D9PTest, correctness) {
  uint32_t n = 516; // nxn + 4
  auto in = generateMatrix(n);
  ...
  stencil_2D_9P(n, in.get(), outBaseline.get());
  stencil_omp_par_simd_2D_9P(n, in.get(), outKernel.get());
